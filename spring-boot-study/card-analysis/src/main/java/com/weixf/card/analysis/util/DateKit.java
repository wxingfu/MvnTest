package com.weixf.card.analysis.util;
...
public class DateKit {
     ...
     * 格式是默认格式
     */
    public static String FORMAR_DEFALUT = "yyyy-MM-dd";
    /**
     * 英文全格式
     */
    ...
    public static String convert2String(Date date) {
        SimpleDateFormat formater = new SimpleDateFormat(FORMAR_LONG);
        try {
            return formater.format(date);
        } catch (Exception e) {
            return null;
    ...
    public static String getCurrentDateDefault() {
        return new SimpleDateFormat(FORMAR_DEFALUT).format(new Date());
    }
...
//            }
     ...
     * @return
     */
    public static List<Date> getDayHoursToDate() {
        Date date = new Date();
        List<Date> dateList = new ArrayList<>();
        date.setTime(System.currentTimeMillis());
        List<String> stringList = getDayHours(date);
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH");
        try {
            for (int i = 0; i < stringList.size(); i++) {
                dateList.add(format.parse(stringList.get(i)));
            }
            return dateList;
